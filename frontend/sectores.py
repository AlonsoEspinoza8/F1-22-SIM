...
class TiemposSectoresPanel(PanelUI):
    ...
    def draw(self, mi_auto):
        self.dibujar_recuadro_universal(250, 650, 400, 200, arcade.color.GRAY)
        arcade.draw_text("Sector | Last Lap | My Best | Race Best", 70, 720, arcade.color.WHITE, 12, bold=True)
        
        if not mi_auto:
            arcade.draw_text("Esperando telemetría...", 70, 690, arcade.color.ORANGE, 11)
            return
